...
// Custom field from TAT clock — comment out if not in your org
...
export default class PrimaryOrderDetail extends LightningElement {
    ...
    // ══════════════════════════════════════════════════════
    // WIRE: compact header record data
    // ══════════════════════════════════════════════════════
    ...
    handleEditSuccess() {
        ...
        // Reset tab caches so they reload with fresh data
    ...
    // ══════════════════════════════════════════════════════
    // ADD PRODUCTS MODAL HANDLERS  (inventoryOrderManager)
    // ══════════════════════════════════════════════════════
    handleOpenInventory() {
    ...
    /**
     * Called when inventoryOrderManager dispatches "productsaved".
     * Closes the modal and refreshes the Order Products tab.
     *
     * NOTE: you need to add one line to inventoryOrderManager.handleSave():
     *   this.dispatchEvent(new CustomEvent('productsaved'));
     * right after the success toast — that's the only change needed there.
     */
    handleInventorySaved() {
        this.showInventoryModal = false;
        // Force products tab to re-fetch on next visit
        this.productsLoaded = false;
        this.orderProducts  = [];
        // If user is already on the products tab, reload immediately
        if (this.activeTab === 'products') {
            this.fetchOrderProducts();
        }
        // Refresh wired header (TotalAmount may have changed)
        notifyRecordUpdateAvailable([{ recordId: this.recordId }]);
        this.dispatchEvent(new ShowToastEvent({
    ...
    // ══════════════════════════════════════════════════════
    // CREATE SUB ORDERS MODAL HANDLERS  (subOrderManager)
    // ══════════════════════════════════════════════════════
    handleOpenSubOrders() {
        this.showSubOrderModal = true;
    }

    handleCloseSubOrders() {
        this.showSubOrderModal = false;
    }

    /**
     * Called when subOrderManager dispatches "suborderscreated".
     * Closes the modal and refreshes the Sub Orders tab.
     *
     * NOTE: you need to add one line to subOrderManager.handleSave() on success:
     *   this.dispatchEvent(new CustomEvent('suborderscreated'));
     * right after the success toast — that's the only change needed there.
     */
    handleSubOrdersCreated() {
        this.showSubOrderModal = false;
        // Force sub-orders tab to re-fetch on next visit
        this.subOrdersLoaded = false;
        this.subOrders       = [];
        // If user is already on sub-orders tab, reload immediately
    ...
    _resetTabCaches() {
        ...
        this.orderProducts   = [];
        this.subOrders       = [];
        // If currently on a data tab, immediately reload it
    ...
    // -- Tab nav items with dynamic classes --
    get tabItems() {
        return [
            {
                id      : 'details',
